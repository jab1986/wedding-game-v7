...
func _ready() -> void:
	...
	# Always ensure default animations exist
...
func _create_default_animations() -> void:
	...
	# Create fade to black animation if it doesn't exist
	if not library.has_animation("fade_to_black"):
		...
		library.add_animation("fade_to_black", fade_black)

	# Create fade from black animation if it doesn't exist
	if not library.has_animation("fade_from_black"):
		var fade_from = Animation.new()
		fade_from.length = 1.0

		var track_idx = fade_from.add_track(Animation.TYPE_VALUE)
		...
		library.add_animation("fade_from_black", fade_from)
